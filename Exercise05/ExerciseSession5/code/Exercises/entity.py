...
from constants import UP, DOWN, LEFT, RIGHT, STOP, TILEWIDTH, SEEK, FLEE, WANDER, WHITE
...
class Entity(object):
    ...
    def goalDirection(self, directions):
        ...
        if self.seek_or_flee:
            for direction in directions:
                vec = (
                    self.node.position
                    + self.directions[direction] * TILEWIDTH
                    - self.goal
                )
    ...
    def getDijkstraPath(self, directions):
        ...
        previous_nodes, shortest_path = dijkstra_or_a_star(
            self.nodes, myTarget, a_star=True
        )
        path = []
        node = lastEnemyNode
        while node != myTarget:
    ...
    def advancedFSM(self):
        ...
        new_state = self.FSM.updateState(
            self.path, coordinates=position, timer=int(self.timer)
        )
        # print(new_state)
        if new_state == SEEK:
            self.speed = 80
